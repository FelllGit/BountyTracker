...
import { useGetW3SecurityContestsAmountByPlatform } from "@/hooks/useGetAmountByPlatform";
...
export function AmountByPlatform() {
  const [activeFilter, setActiveFilter] = useState<EPlatformName | "All">(
    "All"
  );
  const languages = Object.values(EPlatformName);
  const {
    data: rawData,
    isLoading,
    isError,
  } = useGetW3SecurityContestsAmountByPlatform();

  const chartData = useMemo((): TimeSeriesData<EPlatformName>[] => {
    ...
    <Card className="dark:bg-[#30302E] flex flex-col">
      <CardHeader>
        <CardTitle className="mb-2 flex gap-2 items-center">
          Contest Amount Rate By Platform
          <HoverCard>
            <HoverCardTrigger>
              <Info size={15} />
            </HoverCardTrigger>
